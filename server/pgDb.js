...
export const initPostgreDb = () => {
  if (!initPromise) {
    initPromise = (async () => {
      try {
        // 1. Pastikan database sudah dibuat (jika lokal)
        ...
        initPromise = null; // Izinkan retry pada panggilan berikutnya jika sempat gagal
        console.warn(`[PostgreSQL] Catatan: Tidak dapat terhubung ke PostgreSQL (${error.message}).`);
        return false;
      }
    })();
  }
